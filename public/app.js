...
// Your function endpoint (idempotent: safe to call from client)
const ROLL_FN_URL = "https://bainkzbskkqpwpyppyio.functions.supabase.co/roll_daily_words";
// ================================================
...
// ---- IST helpers (server rolls at 00:00 IST) ----
...
function msUntilNextIstMidnight() {
  ...
  return nextIst.getTime() - istNow.getTime();
}
function scheduleIstMidnightRefetch(refetchFn) {
  const wait = Math.max(1000, msUntilNextIstMidnight());
  setTimeout(async () => {
    try { await refetchFn(true); } catch {}
    scheduleIstMidnightRefetch(refetchFn);
  }, wait);
}
...
(async function init() {
  ...
  async function ensureTodayIfMissing(latestDayStr) {
    const istToday = istTodayStr();
    // If we don't have today's row yet, try to trigger the server once (idempotent)
    if (!latestDayStr || latestDayStr < istToday) {
      try {
        await fetch(ROLL_FN_URL, {
          method: 'POST',
          headers: {
        ...
        // ignore; we'll just show the latest available
      }
    }
  }

  async function renderDay(row) {
    dateLabelEl.textContent = new Date(row.day + 'T00:00:00+05:30')
      .toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'short', day: 'numeric' });

    todayWordsEl.innerHTML = '';
    await Promise.all((row.words || []).map(async (w, i) => {
  ...
  async function load(fromMidnightTimer = false) {
    try {
      let days = await fetchDays();

      // If nothing yet, try to seed once (or if we just crossed IST midnight & still no row)
      const latest = days[0]?.day;
      await ensureTodayIfMissing(latest);

      // refetch after trying to ensure
      if (!days.length || (fromMidnightTimer && (!latest || latest < istTodayStr()))) {
        days = await fetchDays();
      }

      if (!days.length) {
        todayWordsEl.innerHTML = `<p class="note">No daily words yet. (Waiting for server to roll today.)</p>`;
        return;
      }

      await renderDay(days[0]);
      await renderHistory(days.slice(1));
      ...
      todayWordsEl.innerHTML = `<p class="note">Error loading daily words: ${err?.message || err}</p>`;
    }
  }

  // initial load + auto-refresh at next IST midnight
  await load();
  scheduleIstMidnightRefetch(load);
})();
